...
namespace boardgame
{
    internal class MySocket
    {
        ...
        public void init()
        {
            ...
            Server.Connect(ep); //인스턴스의 생성자로 입력한 정보들로 서버에 접속 요청.
        ...
        void recv() //서버의 메시지 값을 받는 메소드
        {
            ...
            while (true) //값을 게속 받도록 대기.
            {
                try //에러 발생을 대비해 try-catch사용
                {
                    ...
                    int n = Server.Receive(receiverBuff); //받은 값을 저장하고 그 길이를 n에 저장.
            ...
            // (5) 소켓 닫기
            //Server.Close(); //값을 받는 것이 완벽하게 끝나면 소켓통신종료
        }
        public void Thread_send(string msg) //서버에 보내는 내용이 스택이 쌓여 같이 보내지는 현상 방지.
        {
            Thread Send_Thread = new Thread(new ThreadStart(() => send(msg)));
            Send_Thread.Start();
            //Send_Thread.Join();
        ...
        public void send(string msg) //클라이언트에서 서버로 메시지를 보내기 위한 메소드
        {
            ...
            Server.Send(receiverBuff, 0, receiverBuff.Length, SocketFlags.None); //받은 값과 길이를 서버에 전송.
        }
